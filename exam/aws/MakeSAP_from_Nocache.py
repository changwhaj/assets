...
def open_exam_cache(driver, examno, did):
    exam_url = 'http://webcache.googleusercontent.com/search?q=cache:' + \
                'https://www.examtopics.com/discussions/amazon/view/' + str(did) + \
    ...
    while (driver.title.find('webcache') > 0):
...
def open_exam_site(driver, examno, did):
    exam_url = 'https://www.examtopics.com/discussions/amazon/view/' + str(did) + \
                '-exam-aws-certified-solutions-architect-professional-topic-1/'
    # discuss_url = 'https://www.examtopics.com/ajax/discussion/load-complete/?discussion-id=35981'
    page = driver.get(exam_url)
    sleepsec(10)
    ...
    bs = BeautifulSoup(driver.page_source, 'html.parser')
    div_discuss = bs.find_all("div", {"class": "container outer-discussion-container"})
    while (driver.title.find('AWS Certified Solutions Architect - Professional topic 1 question') < 0):
        # while (len(div_discuss) <= 0):
        print("Waiting...")
        print(driver.title)
        #winsound.PlaySound("SystemExclamation", winsound.SND_ALIAS)
...
if __name__ == "__main__":
    ...
    for i in range(len(ArraySAP))[:]:
        qArray = ArraySAP[i]
        qid = qArray.get('QID')
        did = qArray.get('DID')
        fname = QID + "/" + QID + "-Q" + qid + ".html"
        print(fname)
        ...
        try:
            ...
            if (driver.title.find('AWS Certified Solutions Architect - Professional topic 1 question') > 0):
                with open(fname, "w", encoding='utf-8') as file:
