...
def open_exam(driver, did):
    exam_url = 'http://webcache.googleusercontent.com/search?q=cache:https://www.examtopics.com/discussions/amazon/view/' + \
                str(did) + '-exam-aws-certified-solutions-architect-associate-saa-c02/'
...
def open_local_exam(qid):
    exam_url = "file:///C:/Users/Changwha/GitHub/crawl-assets/SAA/SAA-Q" + str(qid) + ".html"
    page = driver.get(exam_url)
...
if __name__ == "__main__":
    ...
    for i in reversed(range(len(ArraySAA))):
        qSAA = ArraySAA[i]
        qid = qSAA.get('QID')
        did = qSAA.get('DID')
        fname = 'SAA/SAA-Q' + qid + '.html'
        print(fname)
